...
class Index extends Frontend
{
    ...
    public function index()
    {
        ...
        if($qiantao == 1 && $this->is_wxBrowers() == false)
        {
            ...
            unset($p['qiantao']);
            $d = $d . "/index/index/lists?".http_build_query($p);
    ...
    public function vlist()
    {
       ...
       // $userInfo = Admin::getUser($this->id);
        ...
        foreach ($link as &$item)
        {
            ...
            $item['h'] = rand(90 , 99);
            if(in_array($item['id'] , $payedVid['vid']) ||  ($payedVid['is_week'] == 2 || $payedVid['is_date'] == 2 || $payedVid['is_month'] == 2) || $item['try_see'] > 0)
            {
            ...
            else
            {
                ...
                $item['pay'] = 0;
                $item['url'] = $domain ."/index/trading/index?vid={$item['id']}&f={$f}";
            }
    ...
    public function pay($f)
    {
        ...
        $pay = [
            ...
            [
                'name' => "单片购买 {$m} 元",
                'url' => "$domain/index/trading/index?f={$f}&vid={$vid}",
                'flg' => 'dan',
                'money' => 0,
        ...
        if($userInfo['date_fee'] > 0)
        {
            array_push($pay ,[
                ...
                'url' => "$domain/index/trading/index?f={$f}&is_date=2&vid=$vid",
                'flg' => 'date_fee',
        ...
        if($userInfo['week_fee'] > 0)
        {
            array_push($pay ,[
                ...
                'url' => "$domain/index/trading/index?f={$f}&is_week=2&vid=$vid",
                'flg' => 'week_fee',
        ...
        if($userInfo['month_fee'] > 0)
        {
            array_push($pay ,[
                ...
                'url' => "$domain/index/trading/index?f={$f}&is_month=2&vid=$vid",
                'flg' => 'month_fee',
    ...
    protected function getDomain($type = 1)
    {   
