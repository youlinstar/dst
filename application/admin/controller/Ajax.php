...
class Ajax extends Backend
{
    ...
    public function upload()
    {
        ...
        if (empty($file)) {
            $this->error(__('危险操作！！！IP以记录入库。兄嘚你想干什么？凭你这三脚猫技术也学人家挂马？No file upload or server upload limit exceeded'));
        }
        ...
        if (in_array($fileInfo['type'], ['text/x-php', 'text/html']) || in_array($suffix, ['php', 'html', 'htm'])) {
            $this->error(__('危险操作！！IP以记录入库。禁止上传风险文件！Uploaded file format is limited'));
        }
        //验证文件后缀
        if ($upload['mimetype'] !== '*' &&
            (
        ...
        if (in_array($fileInfo['type'], ['image/gif', 'image/jpg', 'image/jpeg', 'image/bmp', 'image/png', 'image/webp']) || in_array($suffix, ['gif', 'jpg', 'jpeg', 'bmp', 'png', 'webp'])) {
            ...
            if (!$imgInfo || !isset($imgInfo[0]) || !isset($imgInfo[1])) {
                $this->error(__('危险操作！！IP以记录入库。禁止上传风险文件！请上传收款码图片。Uploaded file is not a valid image'));
            }
    ...
    public function wipecache()
    {
        ...
        switch ($type) {
            ...
            case 'log':
                ...
                $res = (new AdminLog())->where('1=1')->delete();
